...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Overlay
        ref={ref}
        className={cn(
            // This class string defines the styling for the dialog overlay:
            // - fixed: Positions the overlay fixed relative to the viewport
            // - inset-0: Stretches the overlay to all edges (top:0, right:0, bottom:0, left:0)
            // - z-50: Sets a high z-index to ensure overlay appears above other content
            // - bg-black/80: Semi-transparent black background (80% opacity)
            // Animation classes:
            // - data-[state=open]:animate-in: Triggers animation when dialog opens
            // - data-[state=closed]:animate-out: Triggers animation when dialog closes
